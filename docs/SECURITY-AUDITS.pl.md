...
| R4-ID | 4 | P | `declare_id!` per sieć (mainnet / testnet / placeholder dev), by build bez feature'a nie mógł trafić na wdrożony adres | ✅ | Wdrożone 19.07.2026: testnet `HSCKEu6Q3U96HBzQV6FjbVmhCgeidaBm7uLgC7nG5id7`; placeholder mainnetowy **bez keypaira** (niewdrażalny do ceremonii ID); `programs/anl_staking/src/lib.rs`, `Anchor.toml` |
...
4. Program ID — **`declare_id!` per sieć wdrożone 19.07.2026**: testnet `HSCKEu6Q3U96HBzQV6FjbVmhCgeidaBm7uLgC7nG5id7` (keypair wygenerowany i przechowywany poza repo), placeholder dev-only dla buildów bez feature'a oraz placeholder mainnetowy **bez istniejącego keypaira** (artefakt mainnet kompiluje się, ale jest niewdrażalny do ceremonii Program ID). Na deploy zostaje: kontrola spójności `anchor keys sync` na maszynie buildującej, provider `Anchor.toml` per klaster, ponowna weryfikacja PDA (R1-03 + R4-ID).
