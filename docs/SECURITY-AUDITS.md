...
### 2a. Parallel independent review — Claude Fable 5, 18 Jul 2026

The same evening, a second reviewer independently examined the same snapshot (archived: `docs/audits/2026-07-18_audyt-bezpieczenstwa_claude-fable5.md` + a companion general analysis). Verdict: **no known loss-of-funds paths** — solvency invariants consistent, exit paths unconditional, arithmetic disciplined — with risk concentrated in the **trust and operations layer**. Five medium findings (five low/hardening notes omitted here), tracked as **R1b-M1…M5** in §8:

* **R1b-M1** — `initialize` can be front-run after program deploy (authority takeover) → deployment-procedure item (§9).
* **R1b-M2** — single-key `authority` with no rotation path or timelock (the round-#1 report raised this independently) → pre-mainnet item (§9).
* **R1b-M3** — trust in ANL/XNT mint extensions → **fixed** as R1-04 (extension allowlist gate).
* **R1b-M4** — economic griefing of the reward reservation: a whale can stake a large principal for the maximum period inside Window 1, reserve ~2× principal of Reward-Vault capacity (pushing others into `RewardCoverageExceeded`), then exit via fully refundable `unstake_early` at zero cost — repeatable → open economic-design item (§9).
* **R1b-M5** — missing second-phase admin paths (reward-surplus withdrawal, `genesis_start_ts` correction, documented immutability decisions) → open pre-mainnet design decisions (§9).
...
| R1b-M1 | 1p | M | `initialize` front-run after deploy (authority takeover) | 📋 | Deploy procedure: program deploy → `initialize` → `create_pool` in one atomic session (§9 item 8) |
| R1b-M2 | 1p | M | Single-key authority, no rotation/timelock | 🟡 | Pre-mainnet: two-step `transfer_authority` + real multisig (§9 item 9) |
| R1b-M3 | 1p | M | Trust in ANL/XNT mint extensions | ✅ | Fixed as R1-04 (extension allowlist gate in `initialize`) |
| R1b-M4 | 1p | M | Reward-reservation griefing via fully refundable `unstake_early` | 🟡 | Economic design item; monitored on closed testnet, mitigation decision before mainnet (§9 item 10) |
| R1b-M5 | 1p | M | Missing second-phase admin/recovery paths | 🟡 | Pre-mainnet design decisions (§9 item 11) |
...
8. `initialize` and `create_pool` executed **atomically within the deploy session** (immediately after `solana program deploy`), so the authority role cannot be front-run (R1b-M1).

**Trust & economic layer — pre-mainnet (from the 18 Jul parallel review):**
9. Two-step `transfer_authority` (propose/accept) and a real multisig (e.g. Squads) holding `authority`, plus a key-loss runbook (R1b-M2; raised independently by the round-#1 report).
10. Mitigation decision for reward-reservation griefing (R1b-M4) — options include per-position/per-wallet reservation caps or reservation forfeiture on early unstake; documented acceptance with monitoring is permissible for the closed testnet, a decision is required before mainnet.
11. Second-phase admin paths: reward-surplus withdrawal, a bounded `genesis_start_ts` correction window, and explicitly documented immutability decisions (R1b-M5).
...
| 19 Jul 2026 (later) | Testnet Program ID **rotated before any deploy** (key-hygiene procedure after a seed-phrase exposure during generation; the burned key never touched the network) — current ID: `HSCKEu6Q3U96HBzQV6FjbVmhCgeidaBm7uLgC7nG5id7` |
| 19 Jul 2026 (later) | Original 18 Jul reports recovered from the stray `audyty` branch and archived in `docs/audits/`; the parallel Claude Fable 5 review integrated as R1b-M1…M5 (M-3 fixed via R1-04; M-1 → deploy procedure; M-2/M-4/M-5 open pre-mainnet trust/economic items) |
| 19 Jul 2026 (later) | Per-network `declare_id!` implemented (testnet ID assigned: `HSCKEu6Q3U96HBzQV6FjbVmhCgeidaBm7uLgC7nG5id7`; mainnet: keypair-less placeholder); `Anchor.toml` per-cluster entries; checklist updated |
